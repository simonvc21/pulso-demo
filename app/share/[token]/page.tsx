          ...
          <div className="bg-white rounded-2xl shadow-card border border-line overflow-hidden">
            <div className="bg-navy text-white px-8 py-7">
              ...
              <p className="mt-3 text-sm text-white/80 max-w-2xl">
                The portfolio finished the quarter in strong shape. Total ARR is {fmtPct(qoqArrGrowth, 1)} MoM across {companies.length} companies. Below is the pulse of the fund.
              </p>
            </div>
            <div className="px-8 py-5 grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <SimpleStat label="Portfolio ARR" value={fmtUSD(arrTotal, { compact: true })} hint={`${fmtPct(qoqArrGrowth, 1)} MoM`} positive />
            ...
            <div className="px-8 py-6">
              ...
              <div className="mt-4 prose prose-sm max-w-none text-ink leading-relaxed space-y-3">
                <p className="text-[13.5px]">
                  This was, on balance, a strong quarter. ARR is at <strong>{fmtUSD(arrTotal, { compact: true })}</strong> across the portfolio with a <strong>{fmtPct(qoqArrGrowth, 1)} MoM</strong> growth rate. Top performers continue to be well-capitalized for next-stage conversations later this year.
                </p>
                <p className="text-[13.5px]">
                ...
                <div key={c.name} className="border border-line rounded-xl p-4 bg-paper">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className={`text-[11px] font-medium ${c.qoq >= 0 ? "text-teal-600" : "text-coral"} tabular-nums`}>{fmtPct(c.qoq, 1)} MoM</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-gradient-to-r from-navy to-navy-700 rounded-2xl text-white p-6 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-[13px] leading-relaxed text-white/90">
                {org.name} returned strong results with portfolio ARR up {fmtPct(qoqArrGrowth, 1)} MoM and {yoyGrowth.toFixed(0)}% YoY. The fund is {deployedPct}% deployed across {companies.length} companies. The flagged companies receive active GP support; the rest are tracking ahead of plan.
              </p>
            </div>
          </div>
